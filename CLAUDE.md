...
| `post_likes` / `post_saves` / `post_comments` | per-user marks (own-rows-only reads) and comments (`report_count`, author or post owner may delete) |
...
| `notifications` | the persisted activity feed behind the bell: `kind` (`post_like` / `post_comment` / `mention` / `org_follow` / `org_join_request` / `org_join_accepted` / `org_join_rejected`), actor snapshot, `post_id` / `comment_id` / `org_id`, `snippet`, `read_at`. Written ONLY by definer triggers; users read/flip/delete their own; in the realtime publication |
...
- **Notifications** (`useNotifications`, `notificationService`, `notificationAdapter`): likes, comments, @mentions (in posts and comments) and org follows are trigger-written to `notifications`; the bell count = incoming connections + join requests + unread activity; opening the bell panel, `/notifications`, or the org dashboard reads the activity (rows that were new keep their dot for that visit; there is no separate mark-all button). Org owners see theirs as "Recent activity" on the dashboard. The two legacy kinds stay derived.

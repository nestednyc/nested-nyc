...
- Student: `discover` `events` `detail` `community` `communityPost` `people` `saved` `notifications` `messages` `messageThread` `profile` `userProfile` `create` `edit` `eventDetail` `onboarding` `forgot` `soon`
- Org: `orgSignup` `orgOnboarding` `orgDashboard` `orgEditMe` `orgCommunity` `orgPublic` `orgView` `eventCreate` `eventEdit` `eventResponses`
...
- **Orgs** post *as the org* (`posts.org_id`; RLS lets only a verified org's owner insert it) and get the verified tick + a Follow pill; `org_follows` drives the follower count on the org page and the org's follow notifications (there is no Following feed).
...
- `organizations` INSERT (a club / community, not a seeded university) → "new org waiting for review" → `ADMIN_RECIPIENTS` (falls back to `REPORT_RECIPIENTS`), with the verify SQL inlined — the `notify_organizations` webhook must fire on **Insert and Update**.

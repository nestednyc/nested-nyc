...
| `organizations` | universities + clubs: unique `slug`, `owner_user_id` (the one auth user who runs it — no junction table), `verified` flag (admin-only), `student_run` (trigger-stamped at INSERT when the owner is a `student` account: live without verification, wears a "student-run" label instead of the tick, pinned on UPDATE; a student may own several, ≤ 5, 3 inserts/hr), `spotlight_until` (admin-only), `join_questions` / `join_url`, public `links` JSONB `{kind,url}` rows (legacy `website`/`instagram` kept but no longer written) |
...
- **RLS posture**: anon can read published projects, approved team members, the `public_profiles` view, and **live** orgs/events (`verified OR student_run`; an unverified org-email account's page stays private). Authenticated profile reads are relationship-scoped (self, connections, teammates). Writes are self-scoped; event creation, posting as the org and being followed/joined all require the org to be live and the actor to own it. `public_profiles` and `notif_actor()` wear an org's name/logo **only for `org_admin` accounts** — a student who runs clubs stays themselves everywhere. DM tables are locked harder: direct DML is REVOKEd from `authenticated` — the DEFINER RPCs are the only write path, so idempotency, block checks, and rate limits can't be bypassed.
...
`/community` (students) and `/dashboard/community` (verified orgs) — migrations `20260827000000` → `20260831000002`, domain hook `useCommunity`, screen `community.jsx`, `communityService`, `postAdapter`. Signed-in read only (no anon policy) — Discover stays the public surface.

- **Notes** carry a **kind** — `update` / `win` / `ask` ("looking for") — rendered as a colored kicker; text ≤ 2,000 + ≤ 4 images + an optional project tag (any project the author leads or is approved crew on — `can_tag_project()`, mirrored by `taggableProjects` in the composer; a flyer's **Post to the board** button opens the composer with the project pre-tagged, and the flyer page lists the notes tagged with it under "On the board"); likes, comments (the post author moderates), saves; the board is one newest-first list (no filter chips — every card wears a school badge, `.com-uni`) and saved posts live on `/saved` (`SavedPosts`, `ensureSavedPosts`); "Load older" pages `getFeed({ before })` 30 at a time. Author identity is snapshotted onto the row (`author_*`) because profile reads are relationship-scoped. Rate limits: 10 posts, 60 comments, 60 follows, 20 reports per hour (PT429).
- **Orgs** post *as the org* (`posts.org_id`; RLS lets only a live — verified or student-run — org's owner insert it) and get the verified tick (or the student-run chip) + a Follow pill; `org_follows` drives the follower count on the org page and the org's follow notifications (there is no Following feed).
...
- `organizations` INSERT (a club / community, not a seeded university) → "new org waiting for review" — or, when the row is `student_run`, "new student-run club is live" with the founder's identity and the optional verify / delete SQL — → `ADMIN_RECIPIENTS` (falls back to `REPORT_RECIPIENTS`); the `notify_organizations` webhook must fire on **Insert and Update**.

/* ============================================================
   NESTED NYC — Create a project (pin a flyer to the board)
   category → stage+timeline → identity → recruit → pin
   ============================================================ */
...
import { Av, Facepile, CatTag, Pin } from './shared'
  ...
  function Create({ profile, existingIds, onCancel, onCreate }) {
    ...
    const [roles, setRoles] = useState([{ title: "", note: "", open: true }]);
    const [tags, setTags] = useState([]);
    const [commitment, setCommitment] = useState("");
    const [pinType, setPinType] = useState("tape");
    ...
    const cleanRoles = roles.map((r) => ({ ...r, title: r.title.trim(), note: r.note.trim() })).filter((r) => r.title);
    const canNext = [
      !!cat,
      !!stage && !!timeline.trim(),
    ...
    function finish() {
      ...
      // Phase 2: call projectService.createProject(...) here.
      onCreate({
        id, cat,
        ...
        stage,
        commitment,
        communicationLink: commLink.trim(),
    ...
    if (step === 0) {
      ...
      body = (
        React.createElement("div", { className: "fade-up", key: "c4" },
          React.createElement("span", { className: "onb-kicker" }, "Step 5 · Pin it"),
          React.createElement("h1", null, "Ready to pin?"),
          React.createElement("p", { className: "desc" }, "Pick how it sticks to the board, then send it up. You can re-pin or take it down later."),
          ...
          React.createElement("div", { className: "onb-pitch" },
            ...
            React.createElement("div", { className: "onb-mini-board" },
              React.createElement("div", { className: "mini-flyer", style: { transform: "rotate(-3deg)" } },
                React.createElement("div", { className: "cat-bar", style: { background: cat ? CAT[cat].color : "var(--c-hack)" } }),
                React.createElement("b", null, title.trim() || "Your project"),
                React.createElement("small", null, ((profile && profile.uni && UNI[profile.uni]) ? UNI[profile.uni].name : "Nested") + (cat ? " · " + CAT[cat].label.toLowerCase() : ""))
              ),
            ...
            React.createElement("div", { className: "onb-actions" },
              ...
              step < STEP_COUNT - 1
                ...
                : React.createElement("button", {
                    ...
                    React.createElement(Icon, { name: "pin", size: 17, stroke: "var(--paper)" }),
                    "Pin to the board")
            )
          )
        )

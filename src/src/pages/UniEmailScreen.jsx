...
function UniEmailScreen() {
  ...
  const handleContinue = async () => {
    ...
    // CRITICAL: Validate email FIRST before any Supabase configuration check
    // This ensures Supabase is NEVER called for invalid emails
    const validationError = getEmailValidationError(email)
    if (validationError) {
      setError(validationError)
      return // Stop here - do not proceed to Supabase
    }

    // Only check Supabase configuration after email validation passes
    if (!isSupabaseConfigured()) {
      setError('Authentication service is not configured. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your .env file.')
      return
    }
    ...
    try {
      // Send magic link via Supabase (passwordless auth)
